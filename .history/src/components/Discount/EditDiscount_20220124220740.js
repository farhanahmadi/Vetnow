...
export default function EditDiscount(props) {
    ...
    useEffect( async () =>{
        await axios.get(`${MainLink}/api/v1/products/`).then(res => setData(res.data))
        await axios.get(`${MainLink}/api/v1/discount_update/list/${pros.match.params.id}`).then(res => setData({...data , 
    ...
    return (
        ...
        <form className={styles.main} onSubmit={submitHandler}>
            ...
            <section className={styles.inputs} dir='rtl'>
                ...
                <input type="text" value={data.discountPercent} placeholder="درصد تخفیف" name="subcategoryName" onChange={inputHandler} />
                <input type="text" value={data.valid_from} placeholder="تاریخ شروع تخفیف" name="subcategoryName" onChange={inputHandler} />
                <input type="text" value={data.valid_to} placeholder="تاریخ پایان تخفیف" name="subcategoryName" onChange={inputHandler} />
            </section>
            <br />
