...
export default function DiscountList() {
    ...
    const discountDeleteHandler = (event) =>{
        axios.delete(`${MainLink}/discount_delete/${event.target.value}`).then(response => console.log(response))
        console.log("hi");
    }
    return (
               ...
               <table dir="rtl">
                    ...
                    {data.map(item => <tr key={item.id}> 
                        ...
                        <td>
                            ...
                            <button value={item.id} clickHandler={(event) => discountDeleteHandler(event)}>حذف</button>
                        </td>
                     </tr>)}
                </table>
