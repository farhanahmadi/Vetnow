...
export default function EditDiscount(props) {
    ...
    useEffect( async () =>{
        ...
        await axios.get(`${MainLink}/api/v1/discount_update/${props.match.params.id}/`).then(res => setData({...data , 
        ...
        <form className={styles.main} onSubmit={submitHandler}>
            ...
            <section className={styles.inputs} dir='rtl'>
                ...
                <select  onChange={e => DeleteProductHandler(e)}>
                   ...
                   {/* {products.find(item => item.id === 1 && <option name="discountedProducts" key={item.id} defaultValue="null" value={item.id}>{item.name}</option>)} */}
                   {console.log(products.find(item => item.id === data.product.map(item => item)))}
                   {console.log(data.product.map(item => item == 69))}
