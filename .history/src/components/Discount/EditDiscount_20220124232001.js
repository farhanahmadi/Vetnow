...
export default function EditDiscount(props) {
    ...
    useEffect( async () =>{
        ...
        await axios.get(`${MainLink}/api/v1/discount_detail/${props.match.params.id}/`).then(res => setData({...data , 
            ...
            valid_from: shamsi.gregorianToJalali(res.data.valid_from),
            valid_to: shamsi.gregorianToJalali(res.data.valid_from)
            }))
    ...
    return (
        ...
        <form className={styles.main} onSubmit={submitHandler}>
            ...
            <section className={styles.inputs} dir='rtl'>
                ...
                <input type="text" value={data.va} placeholder="تاریخ شروع تخفیف" name="valid_from" onChange={inputHandler} />
