...
export default function EditDiscount(props) {
    ...
    useEffect( async () =>{
        ...
        await axios.get(`${MainLink}/api/v1/discount_detail/${props.match.params.id}/`).then(res => setData({...data , 
            ...
            valid_from: shamsi.gregorianToJalali(res.data.valid_from).join('-'),
            valid_to: res.data.valid_tores.data.valid_fromshamsi.gregorianToJalali(data.valid_to).join('-')
            }))
    ...
    return (
        ...
        <form className={styles.main} onSubmit={submitHandler}>
            ...
            <section className={styles.inputs} dir='rtl'>
                ...
                <input type="text" value={} placeholder="تاریخ شروع تخفیف" name="valid_from" onChange={inputHandler} />
                <input type="text" value={} placeholder="تاریخ پایان تخفیف" name="valid_to" onChange={inputHandler} />
            </section>
            <br />
