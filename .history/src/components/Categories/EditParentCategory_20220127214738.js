...
const EditParentCategory = () => {
    ...
    useEffect( async () =>{
        await axios.get(`${Main}/api/v1/categories/`).then(res => setCategory(res.data))
    ...
    const submitHandler = (event) =>{
        ...
        axios.put(`${Main}/api/v1/category/update/${data.slug}/` , {
            name: data.parentCategory,
        }).then(res => console.log(res))
    }
    const categoryDeleteHandler =(slug) =>{
        console.log(slug);
        axios.delete(`${Main}/api/v1/category/delete/${slug}/`)
    }
    return (
