...
function App() {
  ...
  useEffect(async () =>  {
    await axios.get(`${MainLink}/api/v1/check/user/per/`,{
      ...
      }).then(response => setIsAdmin(response.data.is_admin) , setIsload(true))
     setIsload(true)
  ...
  return (
    <div className="App">
        ...
        :
        <div>
          {currentLocation != '/' && currentLocation != '/Login-Confirmation' && currentLocation != '/Notfound' && setInterval(isload , 100) &&
