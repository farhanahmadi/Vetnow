...
function App() {
  ...
  const [isNotLoad , setIsNotLoad] = useState(true)
  ...
  useEffect(async () =>  {
    await axios.get(`${MainLink}/api/v1/check/user/per/`,{
      ...
      }).then(response => {setIsAdmin(response.data.is_admin) ; setIsNotLoad(false)})
  ...
  return (
    <div className="App">
        ...
        :
        <div>
          {currentLocation != '/' && currentLocation != '/Login-Confirmation' && currentLocation != '/Notfound' && isNotLoad &&
             <Notfound />
          }
        </div>
